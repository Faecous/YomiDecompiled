...
func process_extra(extra):
	...
	if not (queued_action in invalid_juke_states):
		if juke_dir != null:
...
func tick():
	if not initiative and turn_frames == 0:
	...
	if (juke_ticks > 0 or up_juke_ticks > 0) and hitlag_ticks <= 0 and blockstun_ticks <= 0 and juke_startup_ticks <= 0:
		...
		if up_juke_ticks > 0:
			up_juke_ticks -= 1
			if fixed.lt(juke_dir_y, "0") and current_state().get("IS_JUMP") == null and not (current_state() is ThrowState):
				...
				apply_force("0", UP_JUKE_GRAVITY if not started_up_juke_from_ground else GROUNDED_UP_JUKE_GRAVITY)
		
		if juke_ticks > 0:
			juke_ticks -= 1
			if juke_ticks == JUKE_TICKS + 1 or up_juke_ticks == UP_JUKE_TICKS + 1:
				create_speed_after_image_from_style()
			if not fixed.eq(juke_dir_x, "0"):
