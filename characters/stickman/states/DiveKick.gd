...
func _on_hit_something(obj, hitbox):
	if host.can_divekick_hop and not obj.is_in_group("Fighter") and obj.id == host.id:
...
func _tick():
	if not grounded and current_tick == 3:
		if host.initiative:
			current_tick = 9
	...
	if host.is_grounded() and current_tick > (5 if not grounded else 10):
		host.reset_momentum()
