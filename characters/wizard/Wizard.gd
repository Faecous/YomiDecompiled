...
func init(pos = null):
	...
	if not is_connected("parried", self, "on_parried"):
	...
	if spark_bombs:
		...
		for obj_name in spark_bombs:
			...
			if bomb:
				...
				if not bomb.exploded and bomb.armed and fixed.lt(fixed.vec_len(str(dir.x), str(dir.y)), SPARK_BOMB_PUSH_DISTANCE):
...
func process_extra(extra):
	...
	if extra.has("lock_orb"):
		if orb_projectile:
			...
			if orb:
				if not orb.locked and extra.lock_orb:
					orb.lock()
				elif orb.locked and not extra.lock_orb:
					orb.unlock()
