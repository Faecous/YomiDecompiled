...
func _frame_4():
	...
	if in_place and not foresight and not shift:
		host.start_invulnerability()
	host.start_projectile_invulnerability()
...
func _frame_6():
	...
	if starting_dir != host.get_opponent_dir() and host.combo_count <= 0 and super_level <= 0 and not foresight and not in_place:
