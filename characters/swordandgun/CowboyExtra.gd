...
func update_sight_button():
	sight_button.disabled = not (fighter.supers_available > 0)
	if "Knockdown" in fighter.current_state().state_name:
		sight_button.hide()
...
func update_selected_move(move_state):
	...
	if fighter.after_image_object != null:
		$"%DetonateButton".show()
		update_tp_button()
	sight_button.visible = ( not move_state or (move_state.state_name != "Foresight" and move_state.state_name != "ForesightNeutral")) and fighter.after_image_object == null
	update_sight_button()
	...
	if obj:
		if $"%TpButton".pressed:
			...
			grounded = not fighter.is_grounded()
