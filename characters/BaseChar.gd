...
func block_hitbox(hitbox, force_parry = false, force_block = false, ignore_guard_break = false, autoblock_armor = false):
		...
		if not projectile:
			if current_state().get("IS_NEW_PARRY"):
				...
				parry_timing = turn_frames + (opponent.hitlag_ticks if not projectile else 0)
				...
				var perfect_requirement_no_height = can_perfect_parry() and ( not current_state().get_whiffed_block()) and (opponent.current_state().feinting or opponent.feinting or initiative) and hitbox.parriable
				var perfect_requirement = perfect_requirement_no_height and current_state().matches_hitbox_height(hitbox)



				if is_ghost and perfect_requirement_no_height and not current_state().push and ghost_blocked_melee_attack == - 1:
			...
			perfect_parry = current_state().can_parry and not current_state().get("push") and (always_perfect_parry or host.always_parriable or parried_last_state or (current_state().current_tick < PROJECTILE_PERFECT_PARRY_WINDOW and host.has_projectile_parry_window))
		...
		if perfect_parry:
			...
			if not projectile and not perfect_parry and not last_turn_block and initiative:
				if hitbox.guard_break and not ignore_guard_break and ( not current_state().get_whiffed_block()) and opponent.can_guard_break():
		...
		if not projectile:
			...
			if not current_state().get("IS_NEW_PARRY"):
		...
		if current_state().get("IS_NEW_PARRY"):
			var high_anim = "ParryHigh" if not current_state().use_guard_sprites else "ShieldHigh"
			var low_anim = "ParryLow" if not current_state().use_guard_sprites else "ShieldLow"
			current_state().anim_name = low_anim if (hitbox.hit_height == Hitbox.HitHeight.Low or not is_grounded()) else high_anim
		...
		if not perfect_parry:
			...
			if not projectile:
				...
				if opponent.feints < opponent.num_feints:
					opponent.feints += 1
				if not (hitbox.looping and not hitbox.cancellable) and hitbox.block_cancel_allowed and opponent.can_block_cancel():
					if not hitbox.block_punishable or autoblock_armor:
...
func projectile_free_cancel():
	...
	if not current_state().can_feint_if_possible:
		return 
...
func tick_before():
	...
	if queued_action:
		...
		if queued_action == "Continue":
			...
			elif (was_my_turn or (current_state().interruptible_on_opponent_turn and current_state().next_state_on_hold_on_opponent_turn) or (current_state().hit_fighter and combo_count == 0)) and not feinting and current_state().next_state_on_hold:
