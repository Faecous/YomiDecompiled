...
func has_armor():
	return (armor_active and super_armor_installed and not (current_state() is CharacterHurtState))
...
func tick():
	...
	if landed_move:
		if not (current_state() is CharacterHurtState):
			add_armor_pip()
		landed_move = false
	if is_grounded() and not force_fly:
		flying_dir = null
		if fly_fx_started:
			stop_fly_fx()
	...
	if flying_dir:
		...
		if ( not is_grounded() or force_fly):
	...
	if is_grounded() and not current_state().started_in_air:
		buffer_reset_ground_pound = true
		can_gain_ground_pound = true

	if not can_ground_pound and can_gain_ground_pound and get_pos().y < GROUND_POUND_MIN_HEIGHT and not is_in_hurt_state():
...
func process_extra(extra):
	...
	if extra.has("fly_dir") and ( not is_grounded() or extra.get("force_fly")) and can_fly:
	...
	if extra.has("pull_enabled") and magnet_installed and not buffer_armor:
