...
func is_usable_with_grounded_check(force_aerial = false, force_grounded = false):
	...
	if air_type == AirType.Aerial:
		if force_grounded:
			return false
		if host.is_grounded() and not force_aerial:
			return false
	if air_type == AirType.Grounded:
		if force_aerial:
			return false
		if not host.is_grounded() and not force_grounded:
			return false
	return true
...
func is_usable():
	if host.current_state().state_name == "WhiffInstantCancel":
		...
		if not usable_from_whiff_cancel_if_possible:
			return false
	if uses_air_movement and not (ignore_air_option_use_in_combos and host.combo_count > 0):
...
func enable_interrupt(check_opponent = true, remove_hitlag = false):
	...
	if beats_backdash and host.opponent.current_state().backdash_iasa:
		...
		if not allow_framecheat:
			queue_state_change(fallback_state)
	if hit_fighter and not allow_framecheat:
			queue_state_change(fallback_state)
...
func _on_hit_something(obj, hitbox):
	...
	if count_projectile_hits and not obj.is_in_group("Fighter"):
		hit_anything = true
...
func _can_hit_cancel(obj, hitbox):
	if hitbox.cancellable:
		...
		if obj.is_in_group("Fighter") or can_hit_cancel:
			var projectile = not obj.is_in_group("Fighter")
...
func _tick_shared():
	if current_tick == 0:
		...
		if not is_hurt_state and reversible and not force_same_direction_as_previous_state:
