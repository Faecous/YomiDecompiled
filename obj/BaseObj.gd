...
func is_grounded():
	...
	if ( not grounded and not (current_state().name == "HurtAerial" and float(get_vel().y) <= 0)):
...
func deactivate_current_hitbox():
	for hitbox in get_active_hitboxes():
		if hitbox.active and hitbox.enabled and not hitbox.looping:
			hitbox.deactivate()
...
func compare(obj):
	...
	for property in my_properties:
		...
		if not is_instance_valid(self) or not is_instance_valid(obj):
			return 
		var mine = get(property.name)
		var other = obj.get(property.name)
		if not (mine is Object) and not Utils.compare(mine, other):
